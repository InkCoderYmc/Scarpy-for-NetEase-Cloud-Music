...
from th_3.items import Th3Item
...
class SongCom3Spider(scrapy.Spider):
    name = 'song_com_3'
    allowed_domains = ['music.163.com']
    def start_requests(self):
        song_ids = []
        # 读取每首歌曲对应的歌曲id
        fo = open("D:\\music_163_songs_3.csv", "r", encoding="utf-8")
        for line in fo:
            line = line.replace("\n", "")
            line_s = line.split(',')
            if line_s[0] != '':
        ...
        count_for_get = 228930  # 当前进度表示
        continue_ls = song_ids[count_for_get:250000]
