...
# 读取csv文件，通过id构造url
# 读取每个歌手的所有歌曲保存歌曲id以及歌曲名到新的csv文件
def get_songs(url):
    #headers不用动
    ...
    #每个专辑读取所有歌曲
    for song in soup.find_all('a', href=re.compile(r'/[s][o][n][g][?][i][d][=][0-9][0-9]*')):
        song_name = song.string
        song_id = song['href'].replace('/song?id=', '').strip()
        print("{}, {}".format(song_name, song_id))
        try:
...
ls1 = []
#读取专辑id和名字
fo = open("/home/ymc/Documents/Python project/网易云音乐爬虫及过程文件/requests实现/music_163_albums.csv", "r", encoding='utf-8')
for line in fo:
    line = line.replace("\n", "")
    line_s = line.split(',')
    if line_s[0] != '':
...
count = 0
ls = ls1[count:]
csvfile = open('/home/ymc/Documents/Python project/网易云音乐爬虫及过程文件/requests实现/music_163_songs.csv', 'a', encoding='utf-8')    # 文件存储的位置
writer = csv.writer(csvfile)
for i in ls:
    count = count + 1
    a = count / lent *100
    print("{}/{},{:.3f}%".format(count,lent,a))
    if i != "":
        url = 'https://music.163.com/album?id=' + str(i)
        get_songs(url)
