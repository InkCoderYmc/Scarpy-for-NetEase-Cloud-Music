...
# 读取csv文件，通过id构造url
# 读取每个歌手的所有专辑保存专辑id以及专辑名到新的csv文件
def get_artists(url):
    print('进入歌手页面')
    #headers不用动
    ...
    #每个歌手读取所有专辑
    for album in soup.find_all('a', attrs={'class': 'tit s-fc0'}):
        album_name = album.string
        album_id = album['href'].replace('/album?id=', '').strip()
        print("{}, {}".format(album_name, album_id))
        try:
...
ls_name = []
#读取歌手id和名字
fo = open("/home/ymc/Documents/Python project/网易云音乐爬虫及过程文件/requests实现/music_163_artists.csv", "r", encoding='utf-8')
for line in fo:
    line = line.replace("\n", "")
    line_s = line.split(',')
    if line_s[0] != '':
        ls1.append(line_s[0]) #把歌手id存入ls1
        ls_name.append(line_s[1])
fo.close()
print('读取歌手成功')
csvfile = open('/home/ymc/Documents/Python project/网易云音乐爬虫及过程文件/requests实现/music_163_albums.csv', 'a', encoding='utf-8')    # 文件存储的位置
writer = csv.writer(csvfile)
for i in ls1:
    print('歌手id：{}'.format(i))
    url = 'https://music.163.com/artist/album?id=' + str(i)
    get_artists(url)
